...
| 账号（已脱敏） | 课程 | 说明 |
| --- | --- | --- |
| 132****9876 | 15 门 | 主账号（用户常用） |
| **173****6569** | **企业战略管理** | **本次适配的目标课程，任务中心有 7 个教学任务** |
...
| `docs/任务中心与章节分类.md` | 平台结构分类 + 接口表 + 阻塞点 |
...
1. 用已登录的浏览器（用户本机 Firefox 已登录，账号 173****6569）打开目标页面；
...
**抓完包之后**：按第 6 节的实现落点改代码，补 `tools/probe/` 下对应的探针脚本，跑通后把结论追加到 `docs/任务中心与章节分类.md`。
...
| 第1章 企业战略管理概述 | 2/6 | 视频✅；2 个章节 mooc 侧已刷完但任务引擎未同步（任务 B）；作业+文档锁定 |
...
> 提醒：**整门课刷完需要真实花费数小时**（第1章 3 个视频就 ~36 分钟：696s/787s/682s）。如果继续用 173****6569 这个账号刷，注意不要频繁重跑视频（会重复计算真实时长）。
