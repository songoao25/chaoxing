...
>
> 刷课之外的配套：每次开刷前自动做一次**开始前扫描**（还差什么一眼看清）；AI 写的每条内容**运行中就实时显示**并留痕可查（`./cx review`）；讨论既可以自动刷，也可以**进讨论区自己挑帖子**（`./cx discuss`）。
...
| 功能 | 说明 |
| --- | --- |
| **开始前扫描** | 每次开刷前（默认开启、无需勾选）打印还差什么：章节待刷、教学任务已完成 / 待完成 / 未解锁 / 暂不支持，并提醒漏做的作业与讨论。全程只读，扫描失败不挡刷课 |
| **实时作答留痕** | 答题时逐题显示（`1. 选择  A`、`4. 简答  …（41 字）`）；讨论回复完整显示。同样的内容也会写进普通运行日志 |
| **复核（`./cx review`）** | AI 生成并提交过的文字（测验简答、作业简答、讨论回复、AI实践作答）都留在 `~/.chaoxing/reviews/`，可随时翻阅 |
| **讨论两种模式** | 模式 1：任务里的主题讨论，按课程要求的顺序自动刷。模式 2：`./cx discuss` 列出讨论区，你挑帖子（`1,3,5` / `1-3` / `all`），逐条给草稿、`y/n` 确认后才发送 |
| **安全默认值** | 同时刷 2 个任务（减少验证码）；`--yes` 不会自动群发讨论回复；危险操作的默认值是"否"；管道/非交互输入永远不会把向导卡死 |
...
每一步只问和上一步选择相关的问题；每个环节开始前都会说明自己准备做什么——不重复问、不静默跳过。
...
1. 选用户（第一次使用时新增一个），
2. 登录，
3. 选刷什么——章节、任务中心、两者都刷，或只刷讨论，
4. 选讨论怎么刷——任务里的主题讨论自动刷，还是进讨论区自己挑帖子，
5. 选课程，
6. 每门课刷多少个任务点（选"只刷讨论"时跳过这一步），
7. 确认并开始。
...
数字 = **本次刷多少个「还没完成」的任务点**；已完成的永远跳过，不会重刷。

> 例：一门课 300 节、前 150 节已经刷完。填 `150` 表示"把后面 150 个未完成任务点刷掉"（第 151–300 节），**不是**"只看前 150 节"。

`all`（或直接回车）= 把没完成的全部刷完。章节和教学任务各算各的。

### 开始前扫描（默认开启）

每次开始刷课前自动扫一遍漏刷的东西，不需要勾选、不需要确认：

```text
  开始前扫描
  ──────────────────────────────────────────────
  示例课程
    章节      139 节 · 已完成 37 · 待刷 102，从「1.1 课程导引」接着刷
...
扫描全程只读、不提交任何东西；扫描失败也不会挡住刷课。

### 讨论区：两种模式

| 模式 | 入口 | 做什么 |
| --- | --- | --- |
| 1. 任务里的主题讨论（自动） | 范围选 1/3 时自动包含；或选 4 / `--only-discussion` | 按课程要求的顺序刷任务中心里 `planType=14` 的任务点：读已有回复 → 写一条普通回复 → 提交 |
| 2. 讨论区（自己挑帖子） | 向导范围选 4 → "讨论区"，或 `./cx discuss` | 先只读统计有多少帖子 → 你挑要回复哪几条（`1,3,5` / `1-3` / `all`）→ 逐条给草稿、`y/n` 确认 → 一条一条发送；`--list-topics` 只列不回复 |

两种模式共用同一条链路：读已有回复 → 去 AI 味 → 真人化审计 → 提交 → 实时显示 + `~/.chaoxing/reviews/` 留痕。已经回复过的帖子不会重复发。

向导只问和你的选择相关的问题：只刷章节问章节任务点，只刷任务中心问教学任务，"只刷讨论"不问数量，只问讨论怎么刷（任务里的主题讨论 / 讨论区自己挑帖子）。
...
| 2. 只刷章节 | 只刷目录里的任务点 |
| 3. 只刷任务中心 | 只刷教学任务 |
| 4. 只刷讨论 | 只刷主题讨论（`planType=14`），任务中心里的其它类型本次跳过；也可用 `--only-discussion` |

主题讨论本来就是任务中心里的一类任务点（平台上不是独立入口），所以选 1 或 3 时也会刷讨论。
...
同样的内容也会写进普通运行日志（`~/.chaoxing/chaoxing.log`），不需要额外生成日志文件。
...
```text
~/.chaoxing/
├── config.ini        # 你的配置（答题方式、通知）
├── accounts/         # 按手机号隔离的账号、cookie 与运行配置
├── cache.json        # 题库答案缓存
├── reviews/          # AI 生成并提交过的文字（供复核）
├── submissions.json  # 本地台账：避免重复提交同一份作业
└── chaoxing.log      # 运行日志（默认 DEBUG；CX_LOG_LEVEL=TRACE 看全量）
```
...
| `discussion_mode` | `task` | `task` = 任务里的主题讨论（自动）；`board` = 进讨论区自己挑帖子 |
| `jobs` | `2` | 同时处理几个任务点 |
| `speed` | `2` | 视频倍速（有时长要求的视频始终 1 倍速） |
| `serial_video` | `false` | 视频串行处理（平台进度回退时可打开） |
...
| **AI 实践分数达不到合格线** | 实践成绩由平台自己判。程序用带推理的模型作答并做多次采样投票，但仍可能被判低分；会在配置的轮数内重答。 |
...
```bash
make test      # 离线单测（335 项，不联网、不碰真实账号）
make lint      # 编译检查 + 单测——提交前必跑
make test-313  # 用 CI 同版本（3.13）再跑一遍；本地可能是 3.14
               # （注解求值时机不同，本地绿不代表 CI 绿）
...
| 路径 | 作用 |
| --- | --- |
| `cx` | 推荐入口（`./cx`、`cx setup`、`cx --yes`、`cx discuss`、`cx review`） |
...
| `api/scan.py` | 开始前扫描：把漏刷一次说清 |
| `api/review.py` | AI 内容复核留痕（`cx review`） |
