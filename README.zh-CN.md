...
| 模式 | 入口 | 做什么 |
| --- | --- | --- |
| 1. 任务里的主题讨论（自动） | 范围选 1/3 时自动包含；或选 4 / `--only-discussion` | 按解锁顺序刷任务中心里 `planType=14` 的任务点：读已有回复 → 写一条普通回复 → 提交 |
| 2. 讨论区帖子（自己挑） | 向导范围选 4 → "讨论区帖子"，或 `./cx discuss` | 先只读统计有多少帖子 → 你挑要回复哪几条（`1,3,5` / `1-3` / `all`）→ 逐条给草稿、`y/n` 确认 → 一条一条发送；`--list-topics` 只列不回复 |

两种模式共用同一条链路：读已有回复 → 去 AI 味 → 真人化审计 → 提交 → 实时显示 + `~/.chaoxing/reviews/` 留痕。已经回复过的帖子不会重复发。

向导只问和你的选择相关的问题：只刷章节问章节任务点，只刷任务中心问教学任务，"只刷讨论"不问数量，只问讨论怎么刷（任务里的主题讨论 / 讨论区挑帖）。
